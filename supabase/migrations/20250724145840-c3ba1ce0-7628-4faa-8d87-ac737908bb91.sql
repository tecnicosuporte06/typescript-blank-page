...
-- Inserir dados de exemplo de contatos (sem ON CONFLICT)
INSERT INTO public.contacts (name, phone, email) 
VALUES 
('Daiane Silva', '(21) 98985-5941', 'daiane@email.com'),
('Eva Santos', '(11) 97777-8888', 'eva@acess.com'),
('Igsanara Costa', '(31) 96666-7777', 'igsanara@email.com'),
('Eduardo Lima', '(41) 95555-6666', 'eduardo@email.com'),
('Fulano Pereira', '(51) 94444-5555', 'fulano@email.com');

-- Inserir conversas de exemplo
WITH vendas_queue AS (
  SELECT id FROM public.queues WHERE name = 'Vendas' LIMIT 1
),
sample_contacts AS (
  SELECT id, name FROM public.contacts 
  WHERE name IN ('Daiane Silva', 'Eva Santos', 'Igsanara Costa', 'Eduardo Lima', 'Fulano Pereira')
  LIMIT 5
)
...
SELECT 
  ...
  'contact',
  CASE 
    WHEN cd.name = 'Daiane Silva' THEN 'Oi, gostaria de saber sobre os produtos'
    WHEN cd.name = 'Eva Santos' THEN 'Preciso de mais informações sobre os serviços'
    WHEN cd.name = 'Igsanara Costa' THEN 'Gostaria de agendar uma reunião'
    WHEN cd.name = 'Eduardo Lima' THEN 'Muito interessado no produto'
    ELSE 'Olá, tenho algumas dúvidas'
  END,
  now() - (INTERVAL '2 hours' * cd.rn)
