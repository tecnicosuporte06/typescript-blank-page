-- Phase 1: Database Migration - Rename org to workspace (Fixed dependencies)
...
-- Step 2: Drop the is_member function that depends on org_role
DROP FUNCTION IF EXISTS public.is_member(uuid, org_role);

-- Step 3: Now we can drop the org_role type
DROP TYPE IF EXISTS public.org_role;

-- Step 4: Rename orgs table to workspaces
ALTER TABLE public.orgs RENAME TO workspaces;

-- Step 5: Create workspace_members table (workspace_role enum already exists)
CREATE TABLE public.workspace_members_temp (
...
-- Step 6: Migrate data from org_members to workspace_members_temp
INSERT INTO public.workspace_members_temp (workspace_id, user_id, role, created_at)
SELECT 
...
-- Step 7: Drop old org_members table and rename new one
DROP TABLE public.org_members;
ALTER TABLE public.workspace_members_temp RENAME TO workspace_members;

-- Step 8: Create is_workspace_member function
...
-- Step 9: Rename org_messaging_settings to workspace_messaging_settings
...
-- Step 12: Create workspace-scoped connection functions
...
-- Step 13: Enable RLS and create policies for workspaces
...
-- Step 14: Enable RLS and create policies for workspace_members
...
-- Step 15: Enable RLS and create policies for workspace_messaging_settings
