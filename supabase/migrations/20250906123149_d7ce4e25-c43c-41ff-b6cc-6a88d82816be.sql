-- Phase 1: Database Migration - Rename org to workspace (Fixed)

-- Step 1: Rename orgs table to workspaces
ALTER TABLE public.orgs RENAME TO workspaces;

-- Step 2: Create workspace_members table (workspace_role enum already exists)
CREATE TABLE public.workspace_members_new (
...
-- Step 3: Migrate data from org_members to workspace_members_new
INSERT INTO public.workspace_members_new (workspace_id, user_id, role, created_at)
SELECT 
...
ALTER TABLE public.workspace_members_new RENAME TO workspace_members;

-- Step 5: Update is_workspace_member function
...
-- Step 6: Drop old is_member function
DROP FUNCTION IF EXISTS public.is_member(uuid, org_role);

-- Step 7: Rename org_messaging_settings to workspace_messaging_settings
