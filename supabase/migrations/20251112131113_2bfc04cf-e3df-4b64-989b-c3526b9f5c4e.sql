-- Passo 1: Limpar dados inconsistentes (filas referenciando agentes de outros workspaces)
...
AND NOT EXISTS (
  ...
  WHERE a.id = q.ai_agent_id 
  AND a.workspace_id = q.workspace_id
);

-- Passo 2: Remover constraint antiga
ALTER TABLE public.queues 
DROP CONSTRAINT IF EXISTS queues_ai_agent_id_fkey;

-- Passo 3: Recriar constraint com ON DELETE SET NULL
ALTER TABLE public.queues 
ADD CONSTRAINT queues_ai_agent_id_fkey 
FOREIGN KEY (ai_agent_id) 
REFERENCES public.ai_agents(id) 
ON DELETE SET NULL;

-- Passo 4: Atualizar função delete_workspace_cascade (simplificada)
...
BEGIN
  ...
  -- 6. Delete ai_agents (queues agora ficam com ai_agent_id = NULL automaticamente)
  DELETE FROM public.ai_agents WHERE workspace_id = p_workspace_id;
  
  -- 7. Delete queues (agora sem dependência de ai_agents)
