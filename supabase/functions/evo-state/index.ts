...
serve(async (req) => {
  ...
  try {
    ...
    const evolutionResponse = await fetch(`${EVOLUTION_URL}/instance/connectionState/${instance}`, {
      method: 'GET',
      headers: {
    ...
    console.log(`State response for ${instance}:`, data);

    return new Response(JSON.stringify({ state: data.state || data }), {
      headers: { 
        ...corsHeaders, 
        'Content-Type': 'application/json',
    ...
    console.error('Error in evo-state function:', error);
