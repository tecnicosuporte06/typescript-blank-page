...
export function NovaCampanhaModal(props: {
  open: boolean;
  ...
  const onCreate = async () => {
    if (!props.workspaceId || !props.createdBy) return;
    if (!name.trim()) {
    ...
    try {
      ...
      if (error) throw error;
      if (data?.success === false) throw new Error(data?.error || "Falha ao criar campanha");

      toast({ title: "Campanha criada", description: "Campanha salva e pronta para disparo." });
      props.onCreated?.();
      props.onOpenChange(false);
      reset();
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      toast({ title: "Falha ao criar campanha", description: msg, variant: "destructive" as any });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={props.open} onOpenChange={(o) => { props.onOpenChange(o); if (!o) reset(); }}>
      <DialogContent className="max-w-5xl rounded-none">
            ...
            <Table>
              ...
              <TableBody>
                {loadingContacts ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center text-gray-500 dark:text-gray-300">
                      Nenhum contato encontrado (importe uma lista primeiro).
                    </TableCell>
                  </TableRow>
                ) : (
        ...
        <DialogFooter>
          ...
          <Button className="rounded-none" onClick={onCreate} disabled={loading}>
